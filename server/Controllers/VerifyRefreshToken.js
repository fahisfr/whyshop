...
const RefreshTokenController = (req, res, next) => {
    try {
        ...
        if (!cookie?.refreshtoken)  return res.status(401).json({ status: false, message: 'No token provided.' }) 
        jwt.verify(cookie.refreshtoken, process.env.REFRESH_TOKEN_SECRET, async (err, decoded) => {
            if (decoded) {
                ...
                const accessToken = jwt.sign({ name: FindUser.name, number: FindUser.number, id: FindUser._id, role: FindUser.role }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '30m' });
                res.json({
                    success: true,
                ...
                res.status(401).json({ status:false, message: 'refreshtoken not valid' });
            }
        });
    } catch (error) {
